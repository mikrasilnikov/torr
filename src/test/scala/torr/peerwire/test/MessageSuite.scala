...
object MessageSuite extends DefaultRunnableSpec {
  override def spec =
    suite("MessageSuite")(
      ...
      //
      testM("receive Have") {
        val message = MetaInfoSuite.toBytes("0000000504000000ff")
        val effect  = for {
      ...
      testM("receive BitField") {
        val message = MetaInfoSuite.toBytes("0000000205a0")
        val effect  = for {
          channel    <- InMemoryChannel.make(message)
          buf        <- Buffer.byte(64)
          actual     <- Message.receive(channel, buf)
      ...
      testM("receive Request") {
        val message = MetaInfoSuite.toBytes("0000000d06000000010000000200000003")
        val effect  = for {
          channel <- InMemoryChannel.make(message)
          buf     <- Buffer.byte(64)
          actual  <- Message.receive(channel, buf)
      ...
      testM("receive Cancel") {
        val message = MetaInfoSuite.toBytes("0000000d08000000010000000200000003")
        val effect  = for {
          channel <- InMemoryChannel.make(message)
          buf     <- Buffer.byte(64)
          actual  <- Message.receive(channel, buf)
      ...
      //
      testM("receive Port") {
        val message = MetaInfoSuite.toBytes("0000000309ffff")
        val effect  = for {
          channel <- InMemoryChannel.make(message)
          buf     <- Buffer.byte(64)
          actual  <- Message.receive(channel, buf)
